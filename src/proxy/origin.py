...
async def fetch_origin(path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"An unexpected error occurred during origin fetch for {target_url}: {e}",
            exc_info=True,
        )
...
def _mock_response(path: str) -> Dict[str, Any]:
    """Returns a mock response for the MVP."""
    logger.debug(f"Returning mock response for path: {path}")
    if path.startswith("/static/"):
        return {"content_type": "image/png", "data": {"mock_image": True, "path": path}}
    return {
