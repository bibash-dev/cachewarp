...
from src.proxy.metrics import record_request, observe_request_latency, set_circuit_breaker_state, record_cache_hit, record_cache_miss, record_redis_error
...
class CircuitBreaker:
    ...
    def __init__(self, failure_threshold: int = 3, recovery_timeout: int = 30):
        ...
        set_circuit_breaker_state(self.state)  # Initialize Prometheus metric for circuit breaker state
...
async def caching_middleware(request: Request, call_next: Callable[[Request], Any], cache: Cache, background_tasks: BackgroundTasks) -> Response:
    ...
    """
    start_time = time.time()  # Record the start time of the request for latency measurement
    ...
    if request.url.path in settings.cache_skip_paths:
        ...
        response = await call_next(request)  # Directly forward the request to the next handler
        duration = time.time() - start_time
        observe_request_latency(duration)  # Record the total request processing latency (Prometheus metric)
        return response
    ...
    if "no-cache" in cache_control or "no-store" in cache_control:
        ...
        response = await fetch_and_return(request, cache, None)  # Force fetch from origin
        duration = time.time() - start_time
        observe_request_latency(duration)  # Record latency
        return response
    ...
    cache_key = f"cache:{request.url.path}"  # Key for storing the actual cached response
    lock_key = f"lock:{cache_key}"  # Key for the distributed lock to prevent cache stampedes
    logger.info(f"Processing request: {request.url.path}")

    # --- Step 6: Attempt to Retrieve from Cache ---
    try:
        cached, is_stale = await cache.get(cache_key)  # Try to get the cached response and its staleness status
        logger.debug(f"Cache get result for {cache_key}: cached={cached is not None}, is_stale={is_stale}")
        if cached is not None:
            cache_layer = "L2" if cache.redis else "L1"  # Determine which cache layer served the hit
            record_cache_hit(cache_layer)  # Increment the cache hit metric for the respective layer
            logger.info(f"{'Stale ' if is_stale else ''}Cache hit for: {cache_key}, layer={cache_layer}")
            if is_stale:
            ...
            duration = time.time() - start_time
            observe_request_latency(duration)  # Record latency
            return Response(content=cached, media_type="text/plain", status_code=200)  # Return the cached response
        record_cache_miss("L2" if cache.redis else "L1")  # Increment the cache miss metric
        ...
        response = await call_next(request)  # If cache retrieval fails, forward to origin
        duration = time.time() - start_time
        observe_request_latency(duration)  # Record latency
        return response
        ...
        response = await call_next(request)  # If unexpected cache error, forward to origin
        duration = time.time() - start_time
        observe_request_latency(duration)  # Record latency
        return response

    # --- Step 7: Handle Cache Miss - Acquire Lock for Deduplication ---
    lock_value = await cache.acquire_lock(lock_key, timeout=10)  # Attempt to acquire a distributed lock
    ...
    if lock_value:
        try:
            ...
            if cached is not None:
                ...
                logger.info(f"Cache hit after lock acquisition for: {cache_key}, layer={cache_layer}")
                duration = time.time() - start_time
                observe_request_latency(duration)  # Record latency
        ...
        if cached is not None:
            cache_layer = "L2" if cache.redis else "L1"  # Determine cache layer
            record_cache_hit(cache_layer)  # Record cache hit
            logger.info(f"Cache hit after waiting for lock for: {cache_key}, layer={cache_layer}")
            duration = time.time() - start_time
            observe_request_latency(duration)  # Record latency
            return Response(content=cached, media_type="text/plain", status_code=200)
        logger.warning(f"No cache after waiting for lock, fetching from origin")
        response = await fetch_and_return(request, cache, None)  # Fallback to fetching from origin
        duration = time.time() - start_time
        observe_request_latency(duration)  # Record latency
        return response
...
async def fetch_and_return(request: Request, cache: Cache, cache_key: Optional[str], client_ttl: Optional[int] = None) -> Response:
    ...
    if not circuit_breaker.can_attempt():
        ...
        if cache_key:
            ...
            if cached is not None:
                cache_layer = "L2" if cache.redis else "L1"  # Determine cache layer
                record_cache_hit(cache_layer)  # Record cache hit (for serving stale data)
                logger.info(f"Serving stale data due to circuit breaker OPEN state: {cache_key}, layer={cache_layer}")
                return Response(content=cached, media_type="text/plain", status_code=200)
    ...
    try:
        ...
        if cache_key:
            ...
            if cached is not None:
                cache_layer = "L2" if cache.redis else "L1"  # Determine cache layer
                record_cache_hit(cache_layer)  # Record cache hit (for serving stale data)
                logger.info(f"Serving stale data due to origin failure: {cache_key}, layer={cache_layer}")
                return Response(content=cached, media_type="text/plain", status_code=200)
        return JSONResponse(content={"error": "Service Unavailable"}, status_code=503)
